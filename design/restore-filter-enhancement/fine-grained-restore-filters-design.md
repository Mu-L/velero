...
1. If a `Kind` cannot be resolved via the discovery helper at the start of the restore, Velero stores the raw string as provided in the policy.
...
3. It then checks if this actual `Kind` matches any of the unresolved strings in the user's policy (case-insensitive).
